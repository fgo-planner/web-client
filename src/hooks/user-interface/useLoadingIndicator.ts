...
export const useLoadingIndicator = (): LoadingIndicatorHookResult => {
    ...
    const invokeLoadingIndicator = useCallback((): void => {
        if (!lockIdRef.current) {
            lockIdRef.current = userInterfaceService.requestLock(LockableUIFeature.LoadingIndicator);
        }
    }, [userInterfaceService]);

    const resetLoadingIndicator = useCallback((): void => {
        if (lockIdRef.current) {
            userInterfaceService.releaseLock(LockableUIFeature.LoadingIndicator, lockIdRef.current);
